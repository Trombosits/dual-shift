	...
	if is_instance_valid(_spawner):
		_spawner.wave_started.connect(func(wave_num): 
			...
			if is_instance_valid(_wave_label):
				# --- LOGIKA BOSS SPAWN SETIAP 3 WAVE ---
				if current_wave > 0 and current_wave % 3 == 0:
					_wave_label.text = "BOSS WAVE: " + str(wave_num)
					# NOTE: Jika _spawner butuh di-trigger manual untuk spawn boss,
					# panggil fungsinya di sini. Contoh: _spawner.spawn_boss_wave()
				else:
...
func _input(event):
	...
	if game_over_displayed or get_tree().paused: return
...
func show_end_panel():
	if is_instance_valid(_lose_panel):
		...
		if is_instance_valid(label_target):
			# Karena sudah endless, UI "Menang" kita ubah jadi murni panel skor
